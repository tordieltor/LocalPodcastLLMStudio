...
def validate_safe_output_path(
    ...
    Rejects non-string types, empty/whitespace strings, strings with null bytes or control
    ...
    Raises:
        ValueError: If path is None (and allow_none=False), not a str instance,
                    empty or whitespace-only, contains null bytes/control characters,
    ...
    if any(ord(c) < 32 or ord(c) == 127 for c in clean_path):
        raise ValueError(f"{param_name} contains forbidden control character.")

    # Path traversal ('..') sequence defense
