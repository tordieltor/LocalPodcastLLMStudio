#!/usr/bin/env python3
"""
PodcastStudio Environment Diagnostic Tool
=========================================
...
def check_python_version() -> Dict[str, Any]:
    ...
    return {
        ...
        "remediation": "Install Python 3.10 or newer from https://www.python.org/downloads/ and ensure 'Add python.exe to PATH' is checked." if not is_ok else None
    }
...
def check_virtual_env() -> Dict[str, Any]:
    ...
    return {
        ...
        "active": in_venv,
        "path": venv_path,
        "detail": f"Active ({venv_path})" if in_venv else "Running in global Python (Recommended: run inside .venv)",
        "remediation": "Create and activate virtual environment via 'setup.bat' or '.venv\\Scripts\\activate'." if not in_venv else None
    }
...
def check_packages() -> Dict[str, Any]:
    ...
    return {
        ...
        "remediation": f"Install required dependencies with: pip install -r requirements.txt" if not is_ok else None
    }
...
def check_ollama_service(host: str = "http://localhost:11434", timeout_sec: float = 3.0) -> Dict[str, Any]:
    ...
    try:
        req = urllib.request.Request(
            tags_url,
            headers={"User-Agent": "PodcastStudio-Diagnostic/1.0", "Accept": "application/json"}
        )
        with urllib.request.urlopen(req, timeout=timeout_sec) as response:
            if response.status == 200:
                ...
                return {
                    ...
                    "remediation": "Ollama is running but has no models installed. Pull a model with: ollama pull llama3.1:8b (or ollama pull qwen2.5:7b / mistral-nemo)" if not has_models else None
                }
            else:
                return {
                    "name": "Ollama LLM Service",
...
def run_all_checks(ollama_url: str = "http://localhost:11434") -> Dict[str, Any]:
    ...
    critical_ok = (
        py_check["ok"] and
        pkgs_check["ok"] and
        ollama_check["online"] and
        (ollama_check["models_count"] > 0)
    )
    
    has_warnings = (
        venv_check["warn"] or
        pyinst_check["warn"] or
        ollama_check["warn"] or
        edge_check["warn"]
    )
...
def print_status_line(name: str, status_badge: str, detail: str):
...
def print_diagnostic_report(report: Dict[str, Any]) -> int:
    ...
    print_box("PodcastStudio Preflight Environment Diagnostics")
    ...
    if models:
        # Table Header
        print(f"    {DIM}{'Model Name':<28} {'Size':>9}   {'Params':<8} {'Format':<6} {'Quantization':<12}{RESET}")
        ...
        for m in models:
            ...
            print(f"    {BOLD}{CYAN}{m_name:<28}{RESET} {size_gb:>9}   {param_size:<8} {fmt:<6} {quant:<12}")
    else:
    ...
    if all_passed and not report["has_warnings"]:
        ...
        print(f"  Start PodcastStudio with: {CYAN}{BOLD}python app.py{RESET}\n")
        return 0
    elif all_passed and report["has_warnings"]:
        print(f"  {GREEN}{BOLD}✓ READY WITH WARNINGS:{RESET} Core requirements met, optional items flagged.")
        if remediations:
