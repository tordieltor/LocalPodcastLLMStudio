...
try:
    from core.extractor import extract_text, extract_text_from_file, extract_text_from_pdf
except ImportError:
    # Fallback to direct module if structured differently
    pass
...
class TestExtractorUnit:
    ...
    def test_extract_utf8_text_file(self, sample_text_utf8):
        ...
        assert "PodcastStudio Test Document" in text
    ...
    def test_extract_raw_text_mode(self):
        from core.extractor import extract_text
        raw = "   This is directly pasted raw content about quantum computing and supercomputers.   "
...
class TestExtractorPDFAndEdgeCases:
    ...
    def test_pdf_dehyphenation_and_normalization(self):
        ...
        mock_page = MagicMock()
        mock_page.extract_text.return_value = "This is an infor-\nmation system with   extra    spaces.\n\n\n\nNew section."
    ...
    def test_pdf_scanned_empty_pages_raises_error(self):
        ...
        with patch("pypdf.PdfReader", return_value=mock_reader), \
            ...
            assert "no extractable text" in str(exc_info.value).lower() or "empty" in str(exc_info.value).lower()
