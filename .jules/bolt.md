...
## 2026-09-17 - Single-pass DFS Bucket Classification for Multi-Selector DOM Traversal
**Learning:** Running multiple sequential `_find_nodes` DFS traversals for ordered CSS/tag selectors on parsed DOM trees scales as O(K * N), where K is the number of selectors and N is tree size. Gathering candidate nodes in a single DFS pass into indexed priority buckets reduces traversal complexity to O(N) while preserving selection priority.
**Action:** When selecting DOM nodes matching hierarchical fallback rules, collect candidates in a single recursive tree walk into bucket lists indexed by selector rank.
