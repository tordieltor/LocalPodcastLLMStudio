...
## 2026-08-29 - Avoid Re-joining Token Lists on Repeated Trailing Inspections
**Learning:** Calling `"".join(self._pieces)` on every opening/closing HTML tag to check for trailing newlines created O(N^2) string allocations during document conversion. Scanning `self._pieces` backwards in place reduces trailing newline tracking to O(1).
**Action:** When inspecting state at the end of an accumulated string list, scan the list elements in reverse order rather than re-joining all accumulated pieces.
